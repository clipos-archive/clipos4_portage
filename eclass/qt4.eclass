...
qt4_monolithic_to_split_flag() {
	case ${1} in
		...
		accessibility)
			...
			for y in gui 3support; do
				has_version dev-qt/qt${y} && checkflags+=" dev-qt/qt${y}:${1}"
			done
			;;
		guiaccessibility)
			checkflags+=" dev-qt/qtgui:accessibility"
			;;
		qt3accessibility)
			checkflags+=" dev-qt/qt3support:accessibility"
			;;
...
qt4_pkg_setup() {
	...
	if [[ -n ${QT4_BUILT_WITH_USE_CHECK}${QT4_OPTIONAL_BUILT_WITH_USE_CHECK} ]]; then
		...
		has_version dev-qt/qtcore && local QT44=true
	fi
